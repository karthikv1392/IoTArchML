...
# Class that implements the Combination method that uses the average of SES, damped and Holt for forecasts
...
class Comb_Forecasts():
    ...
    def comb(self):
        ...
        for elem_list in test_set[last_index:]:
            ...
            # Make forecasts using each of the three methods

            fit_damped = Holt(series,damped=True,exponential=True).fit(smoothing_level=0.8, smoothing_slope=0.2,damping_slope=0.1)
            fit_holt = Holt(series, exponential=False).fit(smoothing_level=0.8, smoothing_slope=0.2, optimized=True)
            fit_ses = SimpleExpSmoothing(history_list).fit(smoothing_level=0.3, optimized=False)


            # Applying any the model from the package

            forecast_list_vals_damped = fit_damped.forecast(horizon)
            forecast_list_vals_holt = fit_holt.forecast(horizon)
            forecast_list_vals_ses = fit_ses.forecast(horizon)


            forecast_list_vals_damped = forecast_list_vals_damped.tolist()
            forecast_list_vals_holt = forecast_list_vals_holt.tolist()
            forecast_list_vals_ses = forecast_list_vals_ses.tolist()

            main_array = np.array([forecast_list_vals_damped,forecast_list_vals_holt,forecast_list_vals_ses])
            forecast_array = np.average (main_array,axis=0)
            forecast_list_vals = forecast_array.tolist()
        ...
        pyplot.savefig("./plots/rmse_plot_comb_H30.png", dpi=300)


if __name__ == '__main__':
    ses_forecast_obj = Comb_Forecasts()
    ses_forecast_obj.comb()
